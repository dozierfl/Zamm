...
Complete the remaining Prompt #5 validation with longer sources, multiple seeds per target, section-level drift analysis, and a blinded multi-listener panel. Evaluate candidate-ranking or multi-candidate selection rather than adopting a single provider default from this sweep. Treat Lego as a creative “regenerate track” experiment, not a multitrack reconstruction foundation, unless that broader benchmark overturns this result. Do not build the mixer foundation yet.
