...
export const createGenerationSchema=z.object({prompt:z.string().trim().min(8).max(500),lyrics:z.string().max(10000).optional().default(""),instrumental:z.boolean().optional().default(false),genre:z.string().max(80).optional(),bpm:z.number().int().min(40).max(220).optional(),key:z.string().max(20).optional(),scale:z.enum(["major","minor"]).optional(),durationSeconds:z.number().int().min(1).max(60).optional().default(12),seed:z.number().int().nonnegative().optional(),outputMode:z.enum(["MASTER_ONLY","MULTI_ASSET"]).optional().default("MASTER_ONLY"),providerPolicyAccepted:z.boolean().optional(),vocalProfileId:z.string().uuid().nullable().optional()});
