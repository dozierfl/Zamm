...
export async function GET(request:Request){
 try{
  ...
  const profiles=await sql<Record<string,unknown>[]>`select p.id,p.name,p.status,p.is_private as "isPrivate",p.usable_singing_seconds as "usableSingingSeconds",p.quality_score as "qualityScore",p.range_low_midi as "rangeLowMidi",p.range_high_midi as "rangeHighMidi",p.consented_at as "consentedAt",p.verified_at as "verifiedAt",p.created_at as "createdAt",count(s.id)::int as "sourceCount",(select v.phrase_match_score::float8 from vocal_identity_verifications v where v.profile_id=p.id and v.status='PASSED' order by v.verified_at desc limit 1) as "latestPhraseMatchScore",(select v.id from vocal_profile_versions v where v.profile_id=p.id and v.is_active=true and v.status='READY' and v.revoked_at is null order by v.version_number desc limit 1) as "activeVersionId",(select v.version_number from vocal_profile_versions v where v.profile_id=p.id and v.is_active=true and v.status='READY' and v.revoked_at is null order by v.version_number desc limit 1) as "activeVersionNumber",(select v.provider_model from vocal_profile_versions v where v.profile_id=p.id and v.is_active=true and v.status='READY' and v.revoked_at is null order by v.version_number desc limit 1) as "activeProviderModel" from artist_vocal_profiles p left join vocal_profile_sources s on s.profile_id=p.id where p.owner_id=${user.id} group by p.id order by p.created_at desc`;
  const sources=await sql<{id:string;profileId:string;audioAssetId:string;sourceType:string;originalFilename:string|null;durationSeconds:number;usableDurationSeconds:number;qualityScore:number|null;includedInTraining:boolean;qualityMetrics:Record<string,unknown>;createdAt:string}[]>`select s.id,s.profile_id as "profileId",s.audio_asset_id as "audioAssetId",s.source_type as "sourceType",s.original_filename as "originalFilename",s.duration_seconds as "durationSeconds",s.usable_duration_seconds as "usableDurationSeconds",s.quality_score as "qualityScore",s.included_in_training as "includedInTraining",s.quality_metrics as "qualityMetrics",s.created_at as "createdAt" from vocal_profile_sources s join artist_vocal_profiles p on p.id=s.profile_id where p.owner_id=${user.id} order by s.created_at desc`;
