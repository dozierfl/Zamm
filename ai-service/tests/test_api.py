...
def test_capabilities_do_not_claim_production_multitrack():
    data=client.get("/capabilities").json();assert data["nativeMultitrack"] is False;assert data["masterGeneration"]=="EXPERIMENTAL";assert data["contextualRegeneration"]=="EXPERIMENTAL";assert data["sourceSeparation"]=="UNAVAILABLE"
